                            ...
                            Row(
                                ...
                                Text(
                                    ...
                                    color = when (result.riskLevel) {
                                        "ВЫСОКИЙ РИСК" -> Color.Red
                                        "СРЕДНИЙ РИСК" -> Color(0xFFFFA500) // Оранжевый
                                        "НИЗКИЙ РИСК" -> Color(0xFFFFFF00) // Желтый
                                        else -> Color.Green
                                    }
                                )
                            }
                        ...
                        Column(
                            ...
                            Text(
                                when (result.riskLevel) {
                                    "ВЫСОКИЙ РИСК" -> "🔴 Рекомендуется срочно обратиться к стоматологу для лечения кариеса"
                                    "СРЕДНИЙ РИСК" -> "🟡 Желательно посетить стоматолога в ближайшее время для профилактического осмотра"
                                    "НИЗКИЙ РИСК" -> "🟢 Рекомендуется наблюдение и усиление гигиены полости рта"
                                    else -> "🟢 Продолжайте поддерживать хорошую гигиену полости рта"
                                },
                                style = MaterialTheme.typography.bodyMedium
                            )
                        }
